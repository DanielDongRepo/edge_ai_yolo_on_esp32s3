...
{
    ESP_LOGI("coco_detect", "Loading model: %s", model_name);
    ESP_LOGI("coco_detect", "Model path: %s", path);
    ...
    ESP_LOGI("coco_detect", "Creating Model with param_copy=%d", param_copy);
    ...
    if (m_model == nullptr) {
        ESP_LOGE("coco_detect", "Failed to create model!");
        return;
    }
    
    ESP_LOGI("coco_detect", "Model created successfully");
    ESP_LOGI("coco_detect", "Calling model->minimize()");
    m_model->minimize();
    ESP_LOGI("coco_detect", "Model minimized");
    
    auto outputs = m_model->get_outputs();
    ESP_LOGI("coco_detect", "Model has %d outputs", outputs.size());
    
    int idx = 0;
    for (auto &output : outputs) {
        ESP_LOGI("coco_detect", "Output %d: name=%s", idx++, output.first.c_str());
    }
    
    ESP_LOGI("coco_detect", "Creating ImagePreprocessor");
    ...
    if (m_image_preprocessor == nullptr) {
        ESP_LOGE("coco_detect", "Failed to create ImagePreprocessor!");
        return;
    }
    
    ESP_LOGI("coco_detect", "ImagePreprocessor created, enabling letterbox");
    m_image_preprocessor->enable_letterbox({114, 114, 114});
    
    ESP_LOGI("coco_detect", "Creating yolo11PostProcessor");
    ...
    if (m_postprocessor == nullptr) {
        ESP_LOGE("coco_detect", "Failed to create yolo11PostProcessor!");
        return;
    }
    
    ESP_LOGI("coco_detect", "Yolo11n initialization complete");
}
...
COCODetect::COCODetect(model_type_t model_type, bool lazy_load) : m_model_type(model_type)
{
    ...
    case model_type_t::SIGN_LANGUAGE:
...
void COCODetect::load_model()
{
    ...
    case model_type_t::SIGN_LANGUAGE:
#if CONFIG_FLASH_SIGN_LANGUAGE_MODEL || CONFIG_COCO_DETECT_MODEL_IN_SDCARD
        m_model = new coco_detect::Yolo11n("sign_language_detect_yolo11n_s8_v1.espdl", m_score_thr[0], m_nms_thr[0]);
#else
        ESP_LOGE("coco_detect", "Sign language model is not selected in menuconfig.");
#endif
        break;
    }
}