...
#include "bsp/esp_orderist_v2_bsp.h"
#include "bsp_err_check.h"
#include "bsp/display.h"

static const char *TAG = "ESP-OrderistV2-BSP";
...
/* 只保留 BOOT 按键（GPIO0），移除4个ADC按键以释放GPIO1给LCD MOSI */
/* 原始ADC按键配置已注释保留，不删除，便于回滚 */
/* BSP_BUTTON_INSTALLED_NUM 宏已移至 esp_orderist_v2_bsp.h 头文件，供 main.c 等外部使用 */
static const button_config_t bsp_button_config[BSP_BUTTON_INSTALLED_NUM] = {
    /*
    {
...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    // 增强初始化：针对 RST 接 3.3V (无硬件复位) 的情况，添加软件复位和强制初始化
    ESP_LOGW(TAG, "Performing enhanced LCD initialization (software reset)");
    // 软件复位命令 (0x01)
    esp_lcd_panel_io_tx_param(*ret_io, 0x01, NULL, 0);
    vTaskDelay(pdMS_TO_TICKS(150)); // 必须大于 15ms
    
    // 退出睡眠模式 (0x11) - 对于 GC9A01 尤其重要
    esp_lcd_panel_io_tx_param(*ret_io, 0x11, NULL, 0);
    vTaskDelay(pdMS_TO_TICKS(200)); // 必须大于 120ms
    
    // 打开显示 (0x29)
    esp_lcd_panel_io_tx_param(*ret_io, 0x29, NULL, 0);
...
{
    esp_err_t ret = ESP_OK;
    /* 循环上限改为 BSP_BUTTON_INSTALLED_NUM，原为 BSP_BUTTON_NUM */
    if ((btn_array_size < BSP_BUTTON_INSTALLED_NUM) ||
            (btn_array == NULL)) {
    ...
    for (int i = 0; i < BSP_BUTTON_INSTALLED_NUM; i++) {
